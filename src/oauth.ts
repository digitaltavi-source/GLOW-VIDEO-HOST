...
export type OAuthConfig={issuer:string;audience:string;jwksUrl:string};
export type StaticBearerConfig={token:string;subject:string};
...
export function createJwtVerifier(config:OAuthConfig):OAuthTokenVerifier{
  ...
  return {async verifyAccessToken(token:string):Promise<AuthInfo>{
    try{
      ...
      if(!subject || !exp) throw new OAuthError(OAuthErrorCode.InvalidToken,"missing subject or expiry");
      const rawScope=payload["scope"];
      const rawScp=payload["scp"];
      const scopes=typeof rawScope==="string"
      ...
      const clientId=typeof payload["azp"]==="string"
        ...
        : typeof payload["client_id"]==="string"
          ? payload["client_id"]
          : subject;
      return {token,clientId,scopes,expiresAt:exp,extra:{sub:subject}};
    }catch(error){
